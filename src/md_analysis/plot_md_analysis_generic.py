#!/usr/bin/env python3
"""Plot GROMACS XVG outputs and write generic candidate MD summaries."""
...
def stats(rows: list[tuple[float, float]]) -> dict[str, float | None]:
    ...
    return {"mean": sum(vals) / len(vals), "min": min(vals), "max": max(vals), "last": vals[-1]}
...
def plot_series(output: Path, title: str, ylabel: str, series: list[tuple[str, list[tuple[float, float]]]]) -> None:
    try:
        ...
        print(f"[plot:warn] matplotlib is not available; skipping {output}", flush=True)
        return

    fig, ax = plt.subplots(figsize=(8, 4.5))
    for label, rows in series:
        if rows:
            ax.plot([x for x, _ in rows], [y for _, y in rows], label=label, linewidth=1.6)
    ax.set_title(title)
...
def main() -> int:
    ...
    parser.add_argument("--production-label", default="10 ns")
    ...
    plot_series(output_dir / "rmsd.png", "Backbone RMSD", "RMSD (nm)", [
        ("complex", data["rmsd_complex"]),
        ("antibody", data["rmsd_antibody"]),
        ("antigen", data["rmsd_antigen"]),
    ])
    plot_series(output_dir / "rmsf.png", "Residue RMSF (C-alpha)", "RMSF (nm)", [
        ("complex", data["rmsf_complex_ca"]),
        ("antibody", data["rmsf_antibody_ca"]),
        ("antigen", data["rmsf_antigen_ca"]),
    ])
    plot_series(output_dir / "rg.png", "Radius of Gyration", "Rg (nm)", [
        ("complex", data["rg_complex"]),
        ("antibody", data["rg_antibody"]),
        ("antigen", data["rg_antigen"]),
    ])
    plot_series(output_dir / "sasa.png", "Solvent Accessible Surface Area", "SASA (nm^2)", [
    ...
    plot_series(output_dir / "interface_contacts.png", "Antibody-Antigen Interface Contacts", "Contacts within 0.45 nm", [("contacts", data["interface_contacts"])])
    plot_series(output_dir / "interface_mindist.png", "Antibody-Antigen Minimum Distance", "Distance (nm)", [("minimum distance", data["interface_mindist"])])
    ...
    lines = [
        f"# GROMACS MD summary for {args.candidate_id}",
        "",
        f"- Run ID: {args.run_id}",
        f"- Candidate: {args.candidate_id}",
        "- Input: AF3/PyMOL-confirmed complex structure",
        "- Antibody chains: A+B",
        "- Antigen chain: C",
        f"- Production MD target: {args.production_label}",
